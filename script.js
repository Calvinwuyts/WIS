    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i=0; i < results.length; i++){
                ...
                if (results[i].vegetarian === true){
                    categories += 'vegetarian, '
                }
                if (results[i].vegan === true) {
                    categories += 'vegan, '
                }
                if (results[i].glutenFree === true) {
                    categories += 'gluten free, '
                }
                if (results[i].ketogenic === true) {
                    categories += 'ketogenic, '
                }
                if (results[i].whole30 === true) {
                    categories += 'whole 30, '
                }
                $('#results').append("<div class='result'><div class='resultimgdiv' style='background-image: url("+results[i].image+")'></div><div class='firstinfodiv'><ul><li><a href='"+(results[i].sourceUrl)+"'>"+(results[i].title)+"</a></li><li>"+categories+"</li><li>Ready in: "+(results[i].readyInMinutes)+" minutes</li><li>Cuisine: "+(results[i].cuisines)+"</li><li>Dish type: "+(results[i].dishTypes)+"</li><li>Calories: "+(results[i].calories)+"</li></ul></div><div class='secondinfodiv'></div></div>");
            }
        },
